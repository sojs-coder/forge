...
-   `siblingOf<T extends Part>(...args: string[]): T | undefined`
    Returns the first sibling Part that is of a type included in `args`. `part.siblingOf("Camera", "Input", "Renderer", "BoxCollider")` would return the first Camera, Input, Renderer, TextRender, ColorRender, AnimatedSprite, SpriteRender, Button, or BoxCollider that is a sibling of the part.
